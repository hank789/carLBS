...
var Monitorofflinepage = React.createClass({
    ...
    componentDidMount: function () {
        ...
        // todo 循环加载 暂时先关闭
        setInterval(() => {TrackAction.searchofflineentity();},20000);
    }, 
    onStatusChange: function (type,data) {
        switch (type){
            case 'totalofflinepage':
                this.listenTotalpage(data);
            break;
            case 'offlinelist':
                this.listenofflinelList();
            break;
            case 'initofflinepage':
                this.listenInitpage();
            break;
        }
    },
    ...
    /**
     * 响应Store offlinelist事件，开启翻页按钮可点
     *
     */
    listenofflinelList: function() {
   ...
   /**
    ...
    handleJumppage: function (event) {
        ...
        TrackAction.searchofflineentity(jumpPage);
    ...
    handleLastpage: function (event) {
        if(this.state.currentPage === 1 || this.state.cliking === 1) {
            ...
            TrackAction.searchofflineentity(lastPage);
            this.setState({cliking: 1});
        }
    },
    /**
    ...
    handleNextpage: function (event) {
        if(this.state.currentPage === this.state.totalPage || this.state.cliking === 1) {
            ...
            TrackAction.searchofflineentity(nextPage);
            this.setState({cliking: 1});
        }
    },
    /**
...
export default Monitorofflinepage;
